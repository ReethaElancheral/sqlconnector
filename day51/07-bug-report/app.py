...
from forms import RegisterForm, LoginForm, BugReportForm
...
app.config['SQLALCHEMY_DATABASE_URI'] = "sqlite:///" + os.path.join(BASE_DIR, "instance", "bugs.db")
...
def add_bug():
    form = BugReportForm()
    if form.validate_on_submit():
        bug = BugReport(title=form.title.data, description=form.description.data, reporter=current_user)
        db.session.add(bug)
        db.session.commit()
